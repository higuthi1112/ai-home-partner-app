...
- **★読み上げの声は端末ごとにまったく違う（2026-07-31）★**
  使えるのは端末に入っている音声だけなので、Windows / Android / iPhone で品質が変わる。
  - 以前は `/natural|online|google/` に当たる声を1つ探していたが、
    **iPhone の音声名は「Kyoko」「Otoya」なので1つも当たらず**、
    実際には「最初に見つかった声」が使われていた（品質を選べていなかった）。
  - **iPhone は名前で品質を見分けられない**。拡張版もプレミアム版も同じ名前で並ぶ。
    見分けられるのは `voiceURI` で、**`compact` を含むものが圧縮された低品質版**。
    `services/voicePreference.ts` の `pickDefaultVoice()` はこれを減点している。
  - **★設定画面から本人が選べるようにしてある★**（`voicePreference.ts` + `SettingsMenu`）。
    開発機に入っていない音声は試しようがなく、推測で条件を書いても当たったか確認できない。
    実機で聞き比べて選ぶのが確実。当日おかしいと感じたときの差し替え口にもなる。
    「いまの声」を画面に出しているのは、**iPhoneでは開発者ツールが開けない**ため。
  - `getVoices()` は最初の呼び出しで空を返す環境がある。`voiceschanged` を待つこと。
  - **Polly は入れていない**（日本語には対応している。Kazuha / Tomoko / Takumi が Neural）。
    見送った理由は品質ではなく、**発話のたびに通信待ちが入る**ことと
    **Wi-Fiが落ちると声そのものが出なくなる**こと。今は落ちても声だけは出る。
